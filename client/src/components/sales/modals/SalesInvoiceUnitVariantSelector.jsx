/**
 * SalesInvoiceUnitVariantSelector Component
 * Modal for selecting unit variant (pack/dozen/piece etc) when adding item to Sales Invoice
 * Adapted from GrnUnitVariantSelector for Sales Invoice use
 */
import React, { useState } from "react";
import { X } from "lucide-react";

const SalesInvoiceUnitVariantSelector = ({
  product,
  isOpen,
  onSelect,
  onClose,
  ...
  const unitOptions = [
    ...
    ...packingUnits.map((pu, idx) => ({
      id: `variant-${idx}`,
      name: pu.unit?.unitName || `Unit ${idx + 1}`,  // ✅ Use unit.unitName (e.g., "Pack", "Dozen")
      barcode: pu.barcode || "",
...
export default SalesInvoiceUnitVariantSelector;
